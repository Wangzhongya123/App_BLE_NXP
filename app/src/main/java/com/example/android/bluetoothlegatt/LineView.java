...
public class LineView extends View {
    ...
    private float[] yLables = {0, 2, 4, 6, 8, 10}; //固定y轴数字
    ...
    private int intervalX = dip2px(20); // 坐标刻度的间隔
    private int intervalY = dip2px(20); // y轴刻度的间隔
    private int paddingTop = dip2px(10);// 默认上下左右的padding
    private int paddingLeft = dip2px(30);
    private int paddingRight = dip2px(20);
    private int paddingBottom = dip2px(20);
    private int firstPointX; //第一个点x坐标
    private int xScaleHeight = dip2px(4); // x轴刻度线高度
    private int xyTextSize = sp2px(7); //xy轴文字大小
    private int textToXYAxisGap = dip2px(2); // xy轴的文字距xy线的距离
    ...
    private float maxValueY = 10.0f; // y轴最大值---注意这个值要与Y数组中的最大值一致
    ...
    private String backcolor ="#303030";
    private int backGroundColor = Color.parseColor(backcolor); // view的背景颜色
    ...
    private void initPaint() {
        ...
        paintBack.setColor(Color.parseColor(backcolor));
    ...
    private void drawX(Canvas canvas) {
        ...
        for (int i = 0; i < xValues.size(); i++) {
            ...
            // x轴上的文字
            canvas.drawText(xValues.get(i), firstPointX + i * intervalX - getTextWidth(paintText, "10") / 2,
                    originY + dip2px(10), paintText);
        }
...
//        canvas.save();
        ...
        for (int i = 0; i < yLables.length; i++) {
            canvas.drawText(String.valueOf(yLables[i]), originX - dip2px(15),
                    mHeight - paddingBottom - leftRightExtra - i * intervalY + getTextHeight(paintText, "00.0") / 2, paintText);
        }
